...
public override void LogosSplusInitialize()
{
    ...
    ICHANGED  = new ushort[ 1501 ];
    SDATA  = new CrestronString[ 1501 ];
    for( uint i = 0; i < 1501; i++ )
        SDATA [i] = new CrestronString( Crestron.Logos.SplusObjects.CrestronStringEncoding.eEncodingASCII, 400, this );
    ...
    IN__DOLLAR__ = new InOutArray<StringInput>( 1500, this );
    for( uint i = 0; i < 1500; i++ )
    {
        IN__DOLLAR__[i+1] = new Crestron.Logos.SplusObjects.StringInput( IN__DOLLAR____AnalogSerialInput__ + i, IN__DOLLAR____AnalogSerialInput__, 400, this );
        m_StringInputList.Add( IN__DOLLAR____AnalogSerialInput__ + i, IN__DOLLAR__[i+1] );
    }
    
    OUT__DOLLAR__ = new InOutArray<StringOutput>( 1500, this );
    for( uint i = 0; i < 1500; i++ )
    {
