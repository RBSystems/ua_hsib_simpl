...
namespace PWCSharpPro
{
    public class MTRX_EvertzQuartz : MTRX
    {
        ...
        const int cMaxTries = 0;                // Max number of times a command is sent without an ACK
        public const int IPPORT = 5001;           // The defsault IP port number to connect to  

        //Debug versions
        public const int RxCheckTimeinMs = 1000;
        public const int ListCheckTimeinMs = 500;
        public const int AckCheckTimeinMs = 800;
            ...
            try
            {
                if (_input <= cNumberInputs)
                {
                    if (_output <= cNumberOutputs)
                    {
                        ...
                        {
                            return (string.Format(".SV{1},{0}\x0D", _input, _output));
                        }
                        else if (_signal == Signal.Audio)
                        {
                            return (string.Format(".SA{1},{0}\x0D", _input, _output));
                        }
                        else if (_signal == Signal.Both)
                        {
                            return (string.Format(".SVA{1},{0}\x0D", _input, _output));
                        }
                        else
                        {
