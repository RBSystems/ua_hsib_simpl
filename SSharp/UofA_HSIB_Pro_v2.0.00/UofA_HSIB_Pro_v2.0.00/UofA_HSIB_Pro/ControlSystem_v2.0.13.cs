...
namespace UofA_HSIB_Pro
{
    public class ControlSystem : CrestronControlSystem
    {
        ...
        private void DebugEnableDisable(string _args)
        {
            try
            {
                ...
                switch (args[0].ToUpper())
                {
                    case ("?"):
                        {
                            CrestronConsole.ConsoleCommandResponse("Type the name of the device followed by true or false\re.g. Debug MTRX True");
                            break;
                        }
                    case ("MTRX"):
                        {
        ...
        public void ControlSystem_OnlineStatusChange(GenericBase currentDevice, OnlineOfflineEventArgs args)
        {
            ...
            if (args.DeviceOnLine == true)
            {
                foreach(ThreeSeriesTcpIpEthernetIntersystemCommunications eisc in eiscs)
                {
                    ...
                    {
                        CrestronConsole.PrintLine("{0} *** Ignoring all EISC signals for {1}ms", CLASSID, eiscHandler.PacerTimeinMs);
                        eiscHandler.EnablePacer = true;
                        CTimer timer = new CTimer(StopIgnoring, eiscHandler.PacerTimeinMs);
                        break;
                    }
                }
            }
        ...
        public void StopIgnoring(object _unused)
        {
            eiscHandler.EnablePacer = false;
            CrestronConsole.PrintLine("{0} *** {1}ms passed, receiving all signals normally", CLASSID, eiscHandler.PacerTimeinMs);
        }
