...
namespace UofA_HSIB_Pro
{
    ...
    public class ConfigPacer
    {
        ...
        //private const int CHECKTIMEINMS = 300;                          // Intervals between checking ArgsList in ms
        private const int CHECKTIMEINMS = 1000;                          // Intervals between checking ArgsList in ms
        ...
        private string[,] ConfigStrings = new string[12,3000];          // Mulitdimensional string array to hold config strings from PRO3 for comparision
        ...
        /// </summary>
        public ConfigPacer(SYSM_ConfigurationHandler _handler)
        {
            Handler = _handler;
            ArgsList = new List<ConfigListItem>();
            
            ArgsTimer = new CTimer(ArgsTimerTimedOut, null, CHECKTIMEINMS, CHECKTIMEINMS);
        }
        ...
        {
            if (ArgsList.Count == 0) { return; }    // exit if the list is empty

            if (ArgsList.FindIndex(item => item.method == Method.ConfigureMatrix) != -1)                // If a configuration line for matrix (headend) is in the list
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureMatrix);                      // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureMatrixInput) != -1)      // Matrix input
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureMatrixInput);                 // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureMatrixOutput) != -1)      // Matrix output
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureMatrixOutput);                // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureCamera) != -1)            // Camera
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureCamera);                      // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureDSP) != -1)              // DSP
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureDSP);                         // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureDSPSignal) != -1)        // DSP
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureDSPSignal);                   // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureDisplay) != -1)          // Display
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureDisplay);                     // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureRelay) != -1)            // Relay
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureRelay);                       // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureImageProcessor) != -1)   // Relay
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureImageProcessor);              // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureLight) != -1)            // Lights
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureLight);                       // Get the index
            ...
            else if (ArgsList.FindIndex(item => item.method == Method.ConfigureLightID) != -1)           // Lights ID
            {
                int index = ArgsList.FindIndex(item => item.method == Method.ConfigureLightID);                       // Get the index
        ...
        /// </summary>
        public void AddToList(ConfigPacer.Method _method, SigEventArgs _args)
        {
            //if (_args.Sig.StringValue.Length == 0) { return; }  // If the string value is empty, exit
            //if (ConfigStrings[(int)_method, _args.Sig.Number] == _args.Sig.StringValue) { return; }  // If the string value is the same as before, exit
            ...
            if (ConfigStrings[(int)_method, _args.Sig.Number] == _args.Sig.StringValue) 
            {
        ...
        {
            public ConfigPacer.Method method;
            public SigEventArgs args;

            public ConfigListItem(ConfigPacer.Method _method, SigEventArgs _args)
            {
                method = _method;
                args = _args;
            }
