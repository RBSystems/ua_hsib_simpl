...
namespace UofA_HSIB_Pro
{
    public class ControlSystem : CrestronControlSystem
    {
        ...
        {
            if (Debug) { Print( string.Format("Module {0} wants to send {1}", _module, _command)); }

            if (_module.GetType() == typeof(MTRX_EvertzQuartz))
            {
                if (MTRXEvertzClient.ClientStatus != SocketStatus.SOCKET_STATUS_CONNECTED)
                {
                    if (Debug) { Print( string.Format("{0}_{1}:{2}: *** Not connected, now connecting...", MTRX_EvertzQuartz.CLASSID, MTRXEvertzClient.AddressClientConnectedTo, MTRXEvertzClient.PortNumber, _command)); }
                ...
                {
                    if (Debug) { Print( string.Format("{0}_{1}:{2}: !!! Not Connected", MTRX_EvertzQuartz.CLASSID, MTRXEvertzClient.AddressClientConnectedTo, MTRXEvertzClient.PortNumber, _command)); }
                }
                return;
            }
            ...
            else if (_module.GetType() == typeof(DSP_QSCCore))
            {
                ...
                {
                    if (Debug) { Print( string.Format("{0}_{1}:{2}: *** Not connected, now connecting...", DSP_QSCCore.CLASSID, DSPQSCClient.AddressClientConnectedTo, DSPQSCClient.PortNumber, _command)); }
                ...
                {
                    if (Debug) { Print( string.Format("{0}_{1}:{2}: !!! Not Connected", DSP_QSCCore.CLASSID, DSPQSCClient.AddressClientConnectedTo, DSPQSCClient.PortNumber, _command)); }
                }
                return;
            }
            foreach (Display display in DisplaysList)
            {
                if (display.Controller == _module)
                {
                    ...
                    {
                        if (Debug) { Print( string.Format("{0}{1}_{2}:{3}: *** Not connected, now connecting...", DPLY.ABSCLASSID, display.Guid, display.Client.AddressClientConnectedTo, display.Client.PortNumber, _command)); }
                    ...
                    {
                        if (Debug) { Print( string.Format("{0}{1}_{2}:{3}: !!! Not Connected", DPLY.ABSCLASSID, display.Guid, display.Client.AddressClientConnectedTo, display.Client.PortNumber, _command)); }
                        return;
                    }
                }
            }
        ...
        {
            if (debug) { Print( string.Format("{0}: *** UDP Rx {1} bytes from {2}:{3}", CLASSID, numberOfBytesReceived, myUDPServer.IPAddressLastMessageReceivedFrom, myUDPServer.PortNumber)); }
        ...
        public void TCPClientReceiveCallback(TCPClient myTCPClient, int numberOfBytesReceived)
        {
            try
            {
                if (debug) { Print( string.Format("{0}: *** TCP Rx {1} bytes from {2}:{3}", CLASSID, numberOfBytesReceived, myTCPClient.AddressClientConnectedTo, myTCPClient.PortNumber)); }
        ...
        public void TCPClientSocketStatusChange(TCPClient myTCPClient, SocketStatus clientSocketStatus)
        {
            try
            {
                if (Debug) { Print( string.Format("{0} ControlSystem_SocketStatusChange() *** {1}", myTCPClient, clientSocketStatus)); }
        ...
        public void Print(string _print, bool _TimeStamp)
        {
            if (_TimeStamp)
            {
                Print(DateTime.Now.ToString("HH:mm:ss:fff") + _print);
            }
        }
        ...
        public void Print(string _print)
        {
            ...
            CrestronConsole.PrintLine(_print);
        }
    }
