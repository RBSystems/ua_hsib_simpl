...
namespace PWCSharpPro
{
    public class DPLY_LGLCD : DPLY
    {
         ...
         * v1.0.02: Added IDd versions of power an dinput methods, to allow for individual displays in a daisy chain to be controlled.
         * */
        ...
        public int QuadModePosition = 0;
        public bool QuadModeState = false;
        ...
        {
            string command = string.Format("ka {0:X} 01\x0D", id);
            isPoweredOn = true;
            sendCommand(command);
        }
        ...
        {
            string command = string.Format("ka {0:X} 00\x0D", id);
            isPoweredOn = true;
            sendCommand(command);
        }
        ...
        {
            SwitchtoInput(_input, 0);
        }
        public void SwitchtoInput(DPLY.Input _input, int id)
        {
            string command = "";
            switch (_input)
            {
                case (Input.HDMI1):
                    command = string.Format("xb {0:X} 90\x0D", id);
                    break;
                case (Input.HDMI2):
                    command = string.Format("xb {0:X} 91\x0D", id);
                    break;
                case (Input.VGA):
                    command = string.Format("xb {0:X} 60\x0D", id);
                    break;
            }
            sendCommand(command);
        }
        ...
        public override void TileMatrixMode(int _position)
        {
            ...
            if (_position == 0)
                //tile matrix mode off
                sendCommand(string.Format("dd {0} 00\x0d", DeviceID));
            else
            {
                //this is the command to turn on matrix mode, where "22" means a 2x2 wall
                sendCommand(string.Format("dd {0} 22\x0d", DeviceID));
                //can't remember the code to tell the display which position it is,
                //but this isn't necessary as long as the display ID has been manually set once
                //sendCommand(string.Format("d? {0} {1}\x0d". DeviceID, _position))
            }
        }
        ...
        private void poll(object _args)
        {
            ...
            {
                case (1):
                    command = string.Format("ka 01 FF\x0D");
                    sendCommand(command);
                    break;
                case (2):
                    command = string.Format("xb 01 FF\x0D");
                    sendCommand(command);
                    break;
            }
