...
namespace UofA_HSIB_Pro
{
    public class SYSM_EISCHandler
    {
        ...
        public void EISC_SigChange(BasicTriList currentDevice, SigEventArgs args)
        {
            ...
            {
                parseEvertzSignal(args);
            }
            //else if (Array.IndexOf(DspEiscIds, currentDevice.ID) != -1)
            //{
            //    parseQSCSignal(args);
            //}
        ...
        private void parseEvertzSignal(SigEventArgs args)
        {
            ...
            else if (args.Sig.Type == eSigType.UShort)
            {
                if (args.Sig.Number >= 1 && args.Sig.Number <= 699)
                {
                    if (controlSystem.mtrxSignals != null)
                    {
                        ...
                        if (MYdebug) { CrestronConsole.PrintLine("{0} Queueing Out {1} In {2}", CLASSID, Output, Input); }
        ...
        //private void parseQSCSignal(SigEventArgs args)
        //{
        ...
        //        {
        //            controlSystem.DSPQSC.SetVolume(args.Sig.Number, args.Sig.UShortValue);
        //        }
        //    }
        ...
        private void parseDPLYorCAMSignal(SigEventArgs args)
        {
            ...
            if (args.Sig.Type == eSigType.Bool && args.Sig.BoolValue == true)
            {
                ...
                //{
                //    controlSystem.DisplayList[(int)args.Sig.Number].Control.TogglePower();
                //}
                //// Display Power On
                //else if (args.Sig.Number >= 201 && args.Sig.Number <= 399)
                //{
                //    controlSystem.DisplayList[(int)args.Sig.Number - 200].Control.PowerOn();
                //}
                //// Display Power Off
                //else if (args.Sig.Number >= 401 && args.Sig.Number <= 599)
                //{
                //    controlSystem.DisplayList[(int)args.Sig.Number - 400].Control.PowerOff();
                //}
                //// Display Input 1
                //else if (args.Sig.Number >= 601 && args.Sig.Number <= 799)
                //{
                //    controlSystem.DisplayList[(int)args.Sig.Number - 600].Control.SwitchtoInput(DPLY.Input.HDMI1);
                //}
                //#endregion
                ...
                {
                    controlSystem.CAMSony[args.Sig.Number - 1300].MoveCamera(CAM.Move.Up);
                }
                // Camera down
                else if (args.Sig.Number >= 1351 && args.Sig.Number <= 1399)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].MoveCamera(CAM.Move.Down);
                }
                // Camera left
                else if (args.Sig.Number >= 1401 && args.Sig.Number <= 1449)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].MoveCamera(CAM.Move.Left);
                }
                // Camera right
                else if (args.Sig.Number >= 1451 && args.Sig.Number <= 1499)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].MoveCamera(CAM.Move.Right);
                }
                // Camera right
                else if (args.Sig.Number >= 1500 && args.Sig.Number <= 1549)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].ZoomCamera(CAM.Zoom.In);
                }
                // Camera right
                else if (args.Sig.Number >= 1551 && args.Sig.Number <= 1599)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].ZoomCamera(CAM.Zoom.Out);
                }
                // Camera stop
                else if (args.Sig.Number >= 1701 && args.Sig.Number <= 1749)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].MoveCamera(CAM.Move.Stop);
                }
                #endregion
            }
            ...
            else if (args.Sig.Type == eSigType.UShort)
            {
                ...
                if (args.Sig.Number >= 1301 && args.Sig.Number <= 1349)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1300].MoveToPreset((int)args.Sig.UShortValue);
                }
                // Camera Preset Save
                else if (args.Sig.Number >= 1351 && args.Sig.Number <= 1399)
                {
                    controlSystem.CAMSony[args.Sig.Number - 1350].SavePreset((int)args.Sig.UShortValue);
                }
                #endregion
            }
            else if (args.Sig.Type == eSigType.String)
        ...
        public void UpdateEISCSignal(object device, object value)
        {
            ...
            //{
            //    if (args.Sig.Type == eSigType.String) { CrestronConsole.PrintLine("{0} {1:X} >>> {2} {3} = {4}", CLASSID, currentDevice.ID, args.Sig.Type, args.Sig.Number, args.Sig.StringValue); }
            //    else if (args.Sig.Type == eSigType.String) { CrestronConsole.PrintLine("{0:X} {1} >>> {2} {3} = {4}", CLASSID, currentDevice.ID, args.Sig.Type, args.Sig.Number, args.Sig.UShortValue); }
            //    else if (args.Sig.Type == eSigType.String) { CrestronConsole.PrintLine("{0:X} {1} >>> {2} {3} = {4}", CLASSID, currentDevice.ID, args.Sig.Type, args.Sig.Number, args.Sig.BoolValue); }
            //    else { CrestronConsole.PrintLine("{0} {1} >>> {2} {3}", CLASSID, currentDevice.ID, args.Sig.Type, args.Sig.Number); }
            //}
            ...
            if (device.GetType() == typeof(MTRX_EvertzQuartz))
            {
                ...
                {
                    int output = controlSystem.mtrxSignals.Outputs.FirstOrDefault(x => x.Value.SignalNumber == args.Ouput).Key;
                    int input = controlSystem.mtrxSignals.Inputs.FirstOrDefault(x => x.Value.SignalNumber == args.Input).Key;
                    CrestronConsole.PrintLine("{0} {1:X} >>> Analog {2} = {3}", CLASSID, index, output, input);
                    controlSystem.eiscs[index].UShortInput[(uint)output].UShortValue = (ushort)input;
                }
            }
