...
namespace UofA_HSIB_Pro
{
    public class ControlSystem : CrestronControlSystem
    {
        ...
        public FIRE_RelayHandler FireHandler;
        #endregion
        ...
        public ControlSystem() : base()
        {
            try
            {
                Thread.MaxNumberOfUserThreads = 120;
                ...
                FireHandler = new FIRE_RelayHandler(this);
        ...
        public override void InitializeSystem()
        {
            try
            {
                ...
                new CTimer(FireHandler.CheckID, 2000);
        ...
        public void ModuleNeedsToSend(object _module, string _command)
        {
            ...
            {
                if (Array.IndexOf(RLYGlobalCache, _module) != -1)
                {
                    ...
                    case ("DPLY"):
                        {
                            ...
                            if (args[1].ToUpper() == "ALL")
                            {
                                ...
                                foreach (Display display in DisplaysList)
                                {
                                    ...
                                    CrestronConsole.ConsoleCommandResponse("DPLY {0} debug is now {1}\n", display.Guid, display.Controller.Debug);
                                }
                            }
                            else
                            {
                    ...
                    case ("RLY"):
                        {
                            ...
                            if (args[1].ToUpper() == "ALL")
                            {
                                CrestronConsole.ConsoleCommandResponse("Setting Debug to {0} for all relays...\n", debug);
                                foreach (RLY_GlobalCache relay in RLYGlobalCache)
                                {
                                    relay.Debug = debug;
                                    CrestronConsole.ConsoleCommandResponse("RLY {0} debug is now {1}\n", relay.Name, relay.Debug);
                                }
                            }
                            else
                            {
                                ...
                                CrestronConsole.ConsoleCommandResponse("RLY {0} debug is now {1}\n", index, RLYGlobalCache[index].Debug);
                            }
                            break;
                        }
                }
        ...
        #region COM port and I/O callbacks
        ...
        void port_VersiportChange(Versiport port, VersiportEventArgs args)
        {
            ...
            {
                FireHandler.CheckID(null);
            }
            else if (port.ID >= 1 && port.ID <= 4)
            {
                FireHandler.RecallFireAlarmPreset(port.ID, port.DigitalIn);
            }
        }
        #endregion
