...
namespace UofA_HSIB_Pro
{
    ...
    public class SYSM_EISCHandler
    {
         ...
         * v1.1.02: Pacer only used whilst reconnecting
         * */
        ...
        private object parseIMGPorRLYorLGHTSignal(object _args)
        {
            ...
            {
                if (args.Sig.Number >= 1201 && args.Sig.Number <= 1210)
                {
                    int i = (int)(args.Sig.Number % 50);
                    controlSystem.IMGPTvOne[i].RecallPreset(args.Sig.UShortValue);
                }
            }
            return null;
        }
